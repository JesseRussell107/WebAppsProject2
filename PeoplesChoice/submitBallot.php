...
if (!isset($first) || !isset($second) || !isset($third)) {
    echo "<p class='error'>You didn't vote for all the positions. Grow a backbone, ya pansy!</p>";
} else if ($first == $second || $first == $third || $second == $third) {
    echo "<p class='error'>You moron! You can't make someone first second and third!</p>";
} else {
    ...
    if ($row["Has_Voted"] == 1) {
        echo "<p class='error'>You already voted, ya dingus!</p>";
    } else {
        ...
        mysql_query("UPDATE `cs4220`.`rjpc_vote` SET `Total` = Total + 2 WHERE `rjpc_vote`.`Team_ID` = $second"
                        . " AND `rjpc_vote`.`Project_ID` = $projNum;") or die("Error voting second total");
        ...
        echo "<p>Thank you for your votes.</p>";
